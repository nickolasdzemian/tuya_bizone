...
* iOS -- исправлены ошибки отображения имен датчиков (ENG)
* .4 добавлен программный таймер для вентилятора в режиме климат-контроля
* добавлена возможность скрывать выбранную зону на главном экране
* добавлен сброс значений барабанов по таймауту во всех программных режимах для всех глобальных режимов (добавление, изменение)
...
• тестирование и отладка - обмен данными с сервером

##### исправлены следующие ошибки:

* изменение первой точки в программном режиме для зон - данные не применяются
* изменение первой точки в программном режиме для климата - данные не применяются
* изменение первой точки в режиме программного таймера для зон - данные дублируются
* (iOS) удаление последней точки дня во всех программных режимах - crash (+5)
* (iOS) обработчик анимации единичного пункта списка во всех программных режимах - rowData.item - crash (+5)
* (iOS) обработчик анимации удаленной точки во всех программных режимах - закрыть - crash (+2)
* (iOS) нажатие на пункт меню не закрывает предыдущие открытые в программном режиме (+2)
* (Android) копирование данных на следующий день - при применении первая точка следующего дня дублируется (+2)
* (Android) ожидание прокрутки для отключения меню в программном режиме - не работает (+2)
* во всех программных режимах не работает одновременное закрытие контекстного меню после завершения редактирования точки (+2)
* во всех программных режимах после копирования дня и последующего удаления этих данных - ошибка обработки (+4)

##### дополнительное тестирование

* после простоя/потери связи локальные данные стираются из-за смены ID

#### 3.3.1

* изменено отображение значений таймера в меню настроек кнопок
* изменено определение значений в меню настроек кнопок
* обновлено отображение основного бара навигации на главном экране (стилизация)
* обновлено отображение основного бара в системном навигаторе main
* добавлена фрагментация хранилища и его привязка к v. id устройства
* исправлена работа хранилища
* проведены незначительные внутренние оптимизации и улучшения

** подготовка базы к добавлению темной темы (локальный режим, который распространяется на все терморегуляторы Бизон в приложении)

#### 3.3.0

* отключена возможность включать и настраивать простой таймер в программном режиме(+2)
* отключена возможность выбора и настройки режима программного таймера в многозонном режиме(+2)
* добавлена возможность локального хранения некоторых данных (добавление новых функций, увеличение скорости работы)(+10)
* добавлена возможность добавлять и изменять имя каждой зоны в многозонном режиме
* добавлено отображение меню при первом запуске устройства
* исправлены имена в меню "Управляющие датчики" в настройках зон (#2)
* исправлено отображение даты.времени в подробной статистике за день
* удалены неиспользуемые импорты для плавной работы приложения(-4)
***Меню первого запуска***
* добавлена заставка (изображение устройства) для корректного определения зонирования (height > 700px screens)
* добавлены пункты выбора режима работы устройства
* добавлены ссылки для пропуска меню, перехода в веб-руководство пользователя и перехода в меню настроек прибора
* добавлено отключение меню при выборе любого пункта кроме ссылки на онлайн руководство
* добавлено ограничение на запись иных значений для предотвращения случайного отображения меню

#### 3.2.1

* изменена механика сброса (выкл) простого таймера в многозонном режиме
* изменены иконки в программном режиме
* изменена цветовая схема в многозонном режиме для устройств высотой менее 700px
* переписан контейнер в многозонном режиме для устройств высотой менее 700px на функциональный компонент (speed up)
* внесены корректировки в переводы (+12)
* удалена возможность настройки кнопок на переключение режимов работы
* исправлено отображение статистики при отсутствии данных

#### 3.2.0

* обновлена цветовая схема для недельного программирования в климат-режиме
* добавлено принудительный запрос на размер экрана устройства для переопределения неадаптивных элементов
* добавлен альтернативный метод входа для навигатора (buttons-config)
* исправлено отображение моментального потребления на главном экране
* исправлена ошибка в вычислениях при получении некорректных данных моментального потребления (dp)
* обновлены переводы
* исправлено отображение сообщения об ошибке при выборе некорректного интервала в меню статистики

* исправлена ошибка отображения независимых элементов настройки физических кнопок на устройствах высотой менее 700px, iPhone SE, 5, 5S, 12 mini (xLarge fonts) на главном экране (1-2-2!)
* исправлена ошибка перемещения слайдера при включенной прокрутке

* !! добавлено графическое отображение статистики потребления энергии для каждого реле
* добавлено зависимое от платформы отображение статистики
* уменьшено время обработки при переключении типа графического отображения статистики
* уменьшено кол-во запросов к API (только при изменении данных и загрузке)
***Статистика***
* выполнена доработка существующей функциональной базы (рамки графиков и повторные запросы)
* выполнено объединение графиков для I и II зоны на один экран
* выполнена оптимизация исполнительный функций
* выполнено разделение запросов во избежание конфликта отрисовки и распределения данных
* нулевые значения потребления теперь не отображаются в подробном списке
* увеличена скорость обработки запросов
* добавлена подробная дата в список
...
* добавлена функция динамического масштабирования графиков в зависимости от полученных данных (интервал, макс. знач.)
* добавлена интерполяция для линий в iOS
* исправлена ошибка, которая могла вызывать сбой обновления данных для одновременного отображения двух реле при смене интервала
